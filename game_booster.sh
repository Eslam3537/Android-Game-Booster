  ...
  if [ -d .git ]; then
    if git fetch origin main >/dev/null 2>&1; then
      ...
      if [ -n "$LOCAL_HASH" ] && [ -n "$REMOTE_HASH" ] && [ "$LOCAL_HASH" != "$REMOTE_HASH" ]; then
        msg "$c3" ">>> Update found! Downloading latest version..."
        TEMP_DIR=$(mktemp -d)
        if git clone --depth 1 "$REPO_URL" "$TEMP_DIR" >/dev/null 2>&1; then
          NEW_SCRIPT="$TEMP_DIR/$SCRIPT_NAME"
          if [ -f "$NEW_SCRIPT" ]; then
            chmod +x "$NEW_SCRIPT"
            msg "$c2" ">>> Update downloaded successfully. Restarting with new version..."
            exec "$NEW_SCRIPT"  # إعادة تشغيل السكربت باستخدام الإصدار الجديد
          else
            msg "$c4" ">>> Error: Updated script not found."
            log_error "Updated script not found in $TEMP_DIR"
          fi
          rm -rf "$TEMP_DIR"
        else
          msg "$c4" ">>> Failed to download update. Continuing with current version."
          log_error "Failed to clone updated repository: $REPO_URL"
        fi
      else
    ...
    msg "$c4" ">>> Not a git repository. Initial setup required."
    log_error "Not a git repository during auto-update"
  fi
}
